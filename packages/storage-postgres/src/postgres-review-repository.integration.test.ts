...
for (const fixture of ["review-schema-github-18.sql", "review-schema-local-20.sql"]) {
  test(`PostgreSQL upgrades ${fixture} without changing existing reviews or migration timestamps`, {
    skip: databaseUrl === undefined ? "TEST_DATABASE_URL is not configured" : false,
  }, async () => {
    const schema = `rt_upgrade_${randomBytes(8).toString("hex")}`;
    ...
    try {
      ...
      await pool.query(await readFile(new URL(`./fixtures/${fixture}`, import.meta.url), "utf8"));
      await insertAccount(pool, developer, ["DEVELOPER"]);
      await insertAccount(pool, reviewer, ["REVIEWER"]);
      await pool.query(`
        INSERT INTO rt_review_projects VALUES ('project', 'review-developer', 'project', 'Legacy project', now());
        INSERT INTO rt_review_revisions VALUES ('revision', 'project', 'v1', 'review-developer', now());
        INSERT INTO rt_review_threads (id, revision_id, route_path, anchor_type, anchor, pin_number, body, content_version, author_account_id, created_at, updated_at)
          VALUES ('thread', 'revision', '/', 'REGION', '{"type":"REGION_V1","selection":"POINT","x":0.2,"y":0.3,"width":0,"height":0,"document":{"width":1280,"height":720},"viewport":{"width":1280,"height":720}}', 7, 'Keep this review', 3, 'review-reviewer', now(), now());
        INSERT INTO rt_review_replies (id, thread_id, body, content_version, author_account_id, created_at, updated_at)
          VALUES ('reply', 'thread', 'Keep this reply', 2, 'review-developer', now(), now());
        INSERT INTO rt_review_events (revision_id, route_path, event_type, thread_id, actor_account_id, occurred_at)
          VALUES ('revision', '/', 'COMMENT_CREATED', 'thread', 'review-reviewer', now());
      `);
      const local = fixture.includes("local");
      await pool.query(`INSERT INTO rt_review_notifications (revision_id, route_path, thread_id, content_type, content_id, recipient_account_id, actor_account_id, created_at${local ? ", source_key" : ""})
        VALUES ('revision', '/', 'thread', 'REPLY', 'reply', 'review-reviewer', 'review-developer', now()${local ? ", 'legacy:fixture'" : ""})`);
      if (local) await pool.query("UPDATE rt_review_threads SET status = 'NEEDS_REVIEW', workflow_version = 2 WHERE id = 'thread'");
      const migrations = (await pool.query("SELECT version, applied_at FROM rt_schema_migrations ORDER BY version")).rows;
      const oldThread = (await pool.query("SELECT id, body, content_version, pin_number, status, anchor FROM rt_review_threads")).rows;
      const oldReply = (await pool.query("SELECT * FROM rt_review_replies")).rows;
      const repository = new PostgresReviewRepository(pool);
      await repository.migrate();
      await repository.migrate();
      await repository.checkHealth();
      assert.deepEqual((await pool.query("SELECT id, body, content_version, pin_number, status, anchor FROM rt_review_threads")).rows, oldThread);
      assert.deepEqual((await pool.query("SELECT * FROM rt_review_replies")).rows, oldReply);
      assert.deepEqual((await pool.query("SELECT version, applied_at FROM rt_schema_migrations WHERE version <= $1 ORDER BY version", [local ? 20 : 18])).rows, migrations);
      assert.equal((await pool.query("SELECT max(version) AS version FROM rt_schema_migrations")).rows[0]?.version, 21);
      assert.equal((await pool.query("SELECT count(*)::int AS count FROM rt_review_notifications")).rows[0]?.count, 1);
      assert.equal((await pool.query("SELECT count(*)::int AS count FROM rt_review_events")).rows[0]?.count, 1);
      const service = createReviewService({ repository });
      const detail = await service.getThreadForControl(reviewer, "thread");
      assert.equal(detail.thread.body, "Keep this review");
      assert.equal(detail.thread.replies[0]?.body, "Keep this reply");
      assert.equal((await service.listInbox(reviewer)).notifications.length, 1);
      const created = await service.createRegionComment({ actor: reviewer, controlProjectId: "project", controlRevisionId: "revision", routePath: "/", body: "New pin after upgrade", anchor: detail.thread.anchor });
      assert.equal(created.pinNumber, 8);
    } finally {
