...
HEALTHCHECK --interval=15s --timeout=3s --start-period=10s --retries=3 \
  CMD ["node", "-e", "const h=require('node:http');const q=h.get({host:'127.0.0.1',port:process.env.GATEWAY_PORT||8787,path:'/health/live',headers:{host:process.env.CONTROL_HOST||'control.localhost'}},r=>process.exit(r.statusCode===200?0:1));q.on('error',()=>process.exit(1));q.setTimeout(2500,()=>q.destroy())"]
