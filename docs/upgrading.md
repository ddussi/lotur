...
`v0.1.0-alpha.1`은 아직 게시하지 않은 후보입니다. 아래는 구현된 도구와 운영자가 확인할 절차이며, 새 후보 이미지의 실제 복원 훈련 결과는 [알파 계획](open-source-alpha-plan.md)에 남은 작업으로 관리합니다. 최종 릴리스에는 해당 후보의 검증 기록이 필요합니다.
