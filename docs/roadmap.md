...
| Drafts across reloads | Define storage scope, expiry, sign-out handling, cross-tab conflicts and private-data cleanup. Current draft preservation lasts only in the active tab. |
...
프로젝트별 멤버·초대·접근 권한, 새로고침 후 초안 보존, 여러 Gateway 운영, 외부 알림, 스크린샷 첨부와 전체 편집 이력은 별도 설계가 필요한 후속 범위입니다. 서로 신뢰하지 않는 팀을 같은 서버에 수용하려면 프로젝트 접근 제어가 먼저 필요합니다. 일정은 확정하지 않았습니다.
