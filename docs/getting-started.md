...
[로컬 체험 안내](local-demo.md)는 `npm ci` 후 `npm run demo`로 전용 DB·Gateway·Client·Vite 예제를 함께 시작한다. Node.js 24 이상, 로컬 Docker Engine 28 이상과 Compose, Chrome이 필요하다. 로컬 개발자·검토자 계정을 제공하고 재시작 후에도 제출한 리뷰를 보존한다. 출력되는 주소는 이 컴퓨터에서만 열린다.
