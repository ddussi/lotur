...
> **과거 기록:** 2026-09-09 문서 정리 직전의 2026-09-07 상태 문서를 보존했습니다. 현재 사용·지원 범위는 [구현 현황](../poc-status.md)을 따릅니다.
...
### 2026-09-07 핀 표시·요소 추적 개선 검증

- 핀 전체·개별 표시 전환, 같은 탭의 전역 설정 유지, 다른 검토자와의 표시 상태 분리를 구현했다. 사각 영역 내부는 클릭을 가로채지 않는다.
- 새 핀은 선택 영역을 포함하는 고유한 `data-review-id` 또는 `id`가 있을 때 요소 내부 비율 좌표를 함께 저장한다. 대상이 숨겨지거나 없거나 중복되면 표시를 보류한다. 기존 좌표 핀은 작성 크기를 안내하고 현재 문서 크기·화면 폭이 다르면 숨긴다.
- `npm run check`: 정적 검사·빌드, 스크립트 51개, 애플리케이션·패키지 329개 통과. 별도 PostgreSQL 17.6 컨테이너에 연결했고 건너뛴 검사는 없다. 새 요소 정보의 PostgreSQL 저장·서비스 재생성 후 복원도 확인했다.
- `npm run test:frameworks`: Chrome의 7개 시나리오 통과. 추가한 3개 시나리오는 핀 표시와 SSE·새로고침, 서로 다른 창과 화면 폭에서의 요소 추적·숨김·중복·재등장, 좌표 핀의 레이아웃 차이 안내를 확인한다.
- 이 결과는 로컬 코드 검증이다. 이 변경의 공개 HTTPS 배포·검증 결과는 포함하지 않는다.
