...
초기 기획은 회사 Google Workspace OIDC를 전제로 했지만 제품 소유자는 외부 IdP 없이 관리자가 사용자를 지정해 생성하는 방식을 선택했다. Gateway는 AWS 또는 화면 없는 사내 Linux 서버에서 실행하며, 최초 관리자 생성과 장애 복구에는 브라우저 외 관리 경로도 필요하다.
...
Gateway 재시작 때 Tunnel URL이 끝나는 기존 결정은 유지하지만 내부 계정과 로그인 세션은 PostgreSQL에 남는다. HMAC key 변경 시 기존 로그인·일회용 자격증명은 의도적으로 무효화된다.
