...
1. 리뷰 UI는 Client의 review 모드에서 명시적으로 활성화하는 개발 서버 integration이 bootstrap하는 선택적 오버레이로 제공한다. generic script와 얇은 Vite·Next.js adapter를 제공한다.
...
Phase 1은 page anchor, stable Project·revision, 임시 Tunnel binding, PostgreSQL 저장소, 예약 API와 Shadow DOM sidebar를 구현했다. Phase 2 대화 슬라이스는 plain-text 답글과 Developer의 해결·다시 열기를 추가했다. 리뷰 MVP 완성 단계는 `REGION_V1` 핀·사각 영역, PostgreSQL event log 기반 SSE, 버전 기반 수정·삭제 tombstone, 참여자 멘션·내부 알림과 Vite·Next.js integration을 추가했다.
