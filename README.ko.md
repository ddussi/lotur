...
**현재 상태: 알파.** HTTP·스트리밍·WebSocket 중계, 계정 관리, 임시 공유와 함께 화면 댓글·반응형 핀·필터·리뷰함·내부 알림·재검토를 구현했습니다. 프로젝트별 접근 목록은 아직 없습니다. 소스 패키지 버전은 `0.1.0-alpha.1`입니다. 게시된 버전과 다운로드는 [GitHub Releases](https://github.com/ddussi/lotur/releases)에서 확인합니다. 소스에 적힌 버전만으로 릴리스 게시 여부를 판단하지 않습니다. [현재 구현 상태](docs/poc-status.md)와 [문서 안내](docs/documentation.md)를 참고하세요.
...
Node.js 24 이상, 로컬 Docker Engine 28 이상과 Compose, Chrome을 준비하고 저장소 루트에서 실행합니다.
...
출력된 공유 앱 주소를 열고, 다른 터미널에서 `npm run demo -- credentials`로 검토자·개발자 비밀번호를 확인합니다. Vite 예제와 전용 DB가 포함되며, 종료 후 다시 시작해도 저장된 리뷰가 유지됩니다. 주소는 이 컴퓨터에서만 열립니다. 두 사용자 체험·포트·데이터 삭제는 [로컬 체험 안내](docs/local-demo.md)를 따르세요.
...
연결이 안 되면 같은 명령의 앱 주소 앞에 `doctor`를 넣습니다. 예: `npm run share -- doctor http://127.0.0.1:3000 --gateway wss://control.tunnel.example.com/_review-tunnel/carrier --username developer1`. 로컬 포트, 서버 준비 상태, 로그인·연결 자격 발급을 차례로 확인하고 문제별 해결 방법을 안내합니다. 임시 로그인은 검사 후 종료하며 실제 공유는 시작하지 않습니다. WebSocket 터널과 화면 동작은 공유 후 별도로 확인합니다.
...
리뷰 창 상단에 프로젝트와 기준 버전을 표시합니다. 기준 버전 이후 수정이 있다면 공유 명령에 `--review-changes modified`, 없다면 `--review-changes clean`을 추가합니다. 생략하면 `미확인`입니다. Git 자동 감지가 아닌 **공유 시작 시 개발자 입력**이며, 실행 중인 화면은 이후에도 바뀔 수 있습니다.
...
핀을 모두 숨기거나 댓글 하나의 핀만 표시하고, 번호 버튼으로 해당 위치에 이동할 수 있습니다. 고유한 `data-review-id` 또는 `id`가 있는 요소에 만든 새 핀은 요소의 위치를 따라갑니다. 대상을 찾을 수 없거나 근사 좌표를 현재 화면 크기에 놓을 수 없으면 이유를 표시합니다. 자세한 규칙은 [핀 표시와 위치](docs/getting-started.md#핀-표시와-위치)를 참고하세요.

## 리뷰함에서 작업 이어가기

로그인 주소의 `/reviews`에서 프로젝트·버전·페이지·상태·작성자별로 피드백을 찾습니다. 영구 댓글 링크는 공유가 끝나도 사용할 수 있고, 로그인 후 같은 댓글로 돌아옵니다. 앱이 꺼져 있어도 저장된 대화를 확인하고 허용된 편집을 할 수 있습니다. `앱에서 보기`는 현재 Gateway에서 같은 버전으로 실행 중인 공유를 찾습니다.

재검토 기능을 켜면 개발자가 확인을 요청하고 원댓글 작성자가 해결을 확인하거나 보완을 요청합니다. 리뷰함에는 처리 기록이 남고, 프로젝트 전체의 내 멘션·답글·재검토 알림을 모아 볼 수 있습니다. 사용 흐름과 운영 설정은 [리뷰 사용 안내](docs/internal-review-guide.md)를 따르세요.

실시간 갱신이나 필터 변경 중에는 현재 탭의 초안을 유지합니다. 브라우저가 탭 저장소 사용을 허용하면 새 댓글·답글·수정 초안과 선택한 핀을 새로고침 후에도 복구합니다. 최근 초안은 12시간 동안 같은 주소·탭·로그인·프로젝트·버전 안에서 보관하며, 새 로그인에는 이전 초안을 복구하지 않습니다. 저장·취소한 해당 초안은 지웁니다. 다른 탭·기기로 동기화하지 않으며, 탭 종료 후 복구는 보장하지 않습니다. 저장소 사용이 차단되면 현재 화면에서만 유지합니다.
