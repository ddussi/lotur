...
import type { CommentPage, PublicNotification, ReviewContext } from "./contracts.ts";
import type { RegionReviewAnchorV1 } from "../../../../packages/review/src/index.ts";
...
export function startReviewOverlay(): void {
  ...
  if (document.querySelector(overlayTag) === null) {
    ...
    const apiClient = createReviewApi();
    const mutateReview = apiClient.mutate;
    const drafts = createDraftStore();
    // Thread IDs are non-empty; the empty key belongs to the page's new comment.
    const newCommentDraft = "";
    body.addEventListener("input", () =>
      drafts.write(location.pathname, newCommentDraft, body.value),
    );
    ...
    const anchorFromSelection = (
      start: ReturnType<typeof documentPoint>,
      end: ReturnType<typeof documentPoint>,
    ): RegionReviewAnchorV1 => {
    ...
    const positionSelectionPreview = (
      start: ReturnType<typeof documentPoint>,
      current: ReturnType<typeof documentPoint>,
    ) => {
      ...
      selectionPreview.setAttribute(
        "width",
        String(Math.max(1, Math.abs(current.clientX - start.clientX))),
      );
      selectionPreview.setAttribute(
        "height",
        String(Math.max(1, Math.abs(current.clientY - start.clientY))),
      );
    ...
    selectionLayer.addEventListener("pointerup", (event) => {
      if (selectionStart === undefined) return;
      pendingAnchor = anchorFromSelection(
        selectionStart,
        documentPoint(event.clientX, event.clientY),
      );
      cancelSelection(false);
      setStatus(
        pendingAnchor.selection === "POINT"
          ? "Pin selected; write the comment."
          : "Area selected; write the comment.",
      );
    });
    window.addEventListener(
      ...
      (event) => {
        if (
          event.key === "Escape" &&
          (!selectionLayer.hasAttribute("hidden") || pendingAnchor !== undefined)
        ) {
    ...
    const updateMarkerPositions = () => {
      ...
      for (const marker of markerLayer.querySelectorAll<SVGGElement>(".marker")) {
        const item = commentPage.comments.find(
          (candidate) => candidate.id === marker.dataset.reviewThreadId,
        );
        ...
        if (item.anchor.selection === "RECT") {
          ...
          markerShape?.setAttribute(
            "width",
            String(Math.max(12, item.anchor.width * geometry.width)),
          );
          markerShape?.setAttribute(
            "height",
            String(Math.max(12, item.anchor.height * geometry.height)),
          );
    ...
    const activateThread = (threadId: string, scrollPage: boolean) => {
      activeThreadId = threadId;
      for (const row of comments.querySelectorAll<HTMLLIElement>(".comment")) {
      ...
      if (scrollPage && item?.anchor.type === "REGION_V1") {
        const geometry = documentGeometry();
        window.scrollTo({
          top: Math.max(0, item.anchor.y * geometry.height - innerHeight * 0.25),
          behavior: "smooth",
        });
    ...
    const renderMarkers = () => {
      ...
      for (const item of commentPage.comments) {
        ...
        marker.setAttribute(
          "class",
          `marker ${item.anchor.selection === "POINT" ? "point" : "rect"}`,
        );
        ...
        const openMarker = () => {
          ...
          comments
            .querySelector(`[data-review-thread-id="${item.id}"]`)
    ...
    const renderComments = () => {
      ...
      for (const [id, view] of threadViews) {
        if (!visibleIds.has(id)) {
          view.row.remove();
          threadViews.delete(id);
        }
      }
      for (const [index, item] of items.entries()) {
        let view = threadViews.get(item.id);
        if (view === undefined) {
          ...
          row.append(content);
          view = { row, content };
          threadViews.set(item.id, view);
        }
        const { row, content } = view;
        content.replaceChildren();
        if (comments.children[index] !== row)
          comments.insertBefore(row, comments.children[index] ?? null);
        ...
        threadStatus.textContent =
          item.body === null ? "Deleted" : item.status === "RESOLVED" ? "Resolved" : "Open";
        ...
        content.append(threadHeader, text);
        ...
        if (item.anchor.type === "REGION_V1") {
          ...
          anchorLabel.textContent =
            `Pin #${item.pinNumber}${item.anchor.selection === "POINT" ? " · Point" : " · Area"}`;
        ...
        if (item.replyPageInfo?.hasMore) {
          ...
          olderRepliesButton.addEventListener("click", async () => {
            ...
            try {
              const result = await apiClient.replies(
                item.id,
                routePath,
                item.replyPageInfo?.nextCursor,
              );
              if (sequence !== loadSequence || routePath !== location.pathname) return;
              const seen = new Set(result.replies.map((reply) => reply.id));
              const mergedReplies = [
                ...result.replies,
                ...item.replies.filter((reply) => !seen.has(reply.id)),
              ];
              replaceCommentPage({
                ...commentPage,
                comments: commentPage.comments.map((candidate) =>
              ...
              if (sequence === loadSequence && routePath === location.pathname) {
                setStatus(
                  `Older replies failed: ${error instanceof Error ? error.message : "unknown error"}`,
                  true,
                );
              }
            } finally {
        ...
        if (replies.childElementCount > 0) content.append(replies);

        const canReply =
          context.principal.canComment && item.status === "OPEN" && item.body !== null;
        if (canReply && view.replyForm === undefined) {
          ...
          replyBody.value = drafts.read(routePath, item.id);
          replyBody.addEventListener("input", () =>
            drafts.write(routePath, item.id, replyBody.value),
          );
          ...
          replyForm.addEventListener("submit", async (event) => {
            ...
            const submitted = drafts.capture(routePath, item.id);
            await runMutationAction(replySubmit, "Saving reply…", "Reply failed: ", async () => {
              await mutateReview(`/comments/${encodeURIComponent(item.id)}/replies`, "POST", {
                path: routePath,
                body: submitted.text,
              });
              if (drafts.acknowledge(submitted)) replyBody.value = "";
              await requestLoad();
            });
          });
          row.append(replyForm);
          view.replyForm = replyForm;
          view.replyBody = replyBody;
        }
        if (view.replyForm !== undefined) view.replyForm.hidden = !canReply;
    ...
    const live = createLiveUpdates({
      ...
      reset: () => {
        setStatus("Synchronizing comments…");
        void requestLoad();
      },
      failed: (code) => setStatus(`Live updates unavailable: ${code}`, true),
    });
      ...
      try {
        ...
        const [result, notificationResult] = await Promise.all([
          refreshLoadedPage(
            apiClient,
            routePath,
            commentPath === routePath ? commentPage : emptyCommentPage(),
          ),
        ...
        if (sequence === loadSequence)
          setStatus(
            `Review unavailable: ${error instanceof Error ? error.message : "unknown error"}`,
            true,
          );
      }
    };
    ...
    loadOlderButton.addEventListener("click", async () => {
      ...
      const routePath = location.pathname;
      try {
        const result = await apiClient.page(routePath, commentPage.pageInfo.nextCursor);
        if (sequence !== loadSequence || routePath !== location.pathname) return;
        ...
        if (sequence === loadSequence) {
          setStatus(
            `Older comments failed: ${error instanceof Error ? error.message : "unknown error"}`,
            true,
          );
        }
      } finally {
    ...
    form.addEventListener("submit", async (event) => {
      event.preventDefault();
      const routePath = location.pathname;
      const submitted = drafts.capture(routePath, newCommentDraft);
    ...
    window.addEventListener("resize", updateMarkerPositions, { signal: lifetime.signal });
    watchNavigation(() => {
      activeThreadId = undefined;
      body.value = drafts.read(location.pathname, newCommentDraft);
      cancelSelection(true);
      live.close();
    ...
    window.addEventListener(
      "pageshow",
      (event) => {
        if (event.persisted) void requestLoad();
      },
      { signal: lifetime.signal },
    );
