import { REVIEW_CONTROL_SHARED_SOURCE } from "./review-control.generated.ts";
...
export const REVIEW_CONTROL_SOURCE = REVIEW_CONTROL_SHARED_SOURCE + `
(() => {
  const lifetime = new AbortController();
  window.addEventListener("pagehide", event => { if (!event.persisted) lifetime.abort(); }, { signal: lifetime.signal });
  ...
  const read = async (path, options) => {
    ...
    if (response.status === 401 || response.status === 403) {
      ++accessGeneration; clearDrafts(); document.querySelector("#notifications").replaceChildren();
    ...
    if (generation !== accessGeneration || lifetime.signal.aborted) throw new Error("REVIEW_FORBIDDEN");
  ...
  const inboxTimer = setInterval(() => { if (!document.hidden && !document.querySelector("#inbox").open) void refreshInbox(false); }, 2000);
  lifetime.signal.addEventListener("abort", () => clearInterval(inboxTimer), { once: true });
  const run = async () => {
    const parts = location.pathname.split("/");
    if (parts[2] === "threads" && parts[3]) {
      const endpoint = "/comments/" + encodeURIComponent(parts[3]);
      let detail;
      let page = ReviewTunnelUi.emptyCommentPage();
      const heading = node("div");
      const list = node("ol");
      main.append(heading, list);
      const reload = ReviewTunnelUi.createRefreshCoordinator(async () => {
      ...
      ReviewTunnelUi.attachMentions(main, prefix => read(endpoint + "/mentions?prefix=" + encodeURIComponent(prefix)), lifetime.signal);
      const view = ReviewTunnelUi.createThreadView({ container: list, getPage: () => page, getContext: () => detail,
      ...
      const threadTimer = setInterval(() => { if (!document.hidden) reload().catch(error => showStatus(error.message, true)); }, 1500);
      lifetime.signal.addEventListener("abort", () => clearInterval(threadTimer), { once: true });
      return;
    }
